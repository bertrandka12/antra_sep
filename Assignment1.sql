...
--3. Write a query that retrieves the columns ProductID, Name, Color and ListPrice from the Production.Product table, the rows that are NULL for 
--the Color column.
SELECT p.ProductID, p.Name, p.Color, p.ListPrice
From Production.Product p
WHERE p.Color IS NULL

--4. Write a query that retrieves the columns ProductID, Name, Color and ListPrice from the Production.Product table, the rows that are not NULL for 
--the Color column.
SELECT p.ProductID, p.Name, p.Color, p.ListPrice
From Production.Product p
WHERE p.Color IS NOT NULL

--5. Write a query that retrieves the columns ProductID, Name, Color and ListPrice from the Production.Product table, the rows that are not NULL for 
--the column Color, and the column ListPrice has a value greater than zero.
SELECT p.ProductID, p.Name, p.Color, p.ListPrice
From Production.Product p
...
SELECT ('NAME: '+p.Name+' -- '+ 'COLOR: '+p.Color) NameColor 
FROM Production.Product AS p 
WHERE (p.Name like '%Crankarm' OR p.Name like 'Chainring%') AND p.Color IN ('Silver', 'Black')
...
--12.  Write a query that retrieves the columns Name and ListPrice from the Production.Product table. Your result set should look something like 
...
--13. Write a query so you retrieve rows that have a Name that begins with the letters SPO, but is then not followed by the letter K. After this zero or 
